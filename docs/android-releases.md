...
FleetFlow can publish signed Android APK and AAB artifacts to GitHub Releases.

## What visitors get

- A downloadable Android APK attached to a GitHub Release for direct sideload installs
- A signed Android App Bundle (`.aab`) attached to the same release for Play Store style distribution
...
4. GitHub Actions builds a signed APK and AAB, then attaches both to the matching GitHub Release.

## Notes

- Visitors should install the `.apk`; the `.aab` is intended for store distribution workflows rather than direct device installs.
